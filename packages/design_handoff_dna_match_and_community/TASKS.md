...
- [ ] **1.2 Typography.** §4. Add `fontFamily` and `textTransform` to `TypographyStyle`, set the `meta` role to monospace uppercase, register the font in `load-fonts.ts`, apply in `components/text.tsx`. Step any 600/700 weights down. — _0.1: **bigger than it reads.** (a) `fontWeight`'s union must widen **down** to `'200'|'300'|'400'|'500'` or weight 300 is unusable — §4's original snippet contradicted its own prose. (b) `load-fonts.ts` is a no-op stub and `assets/fonts/` is empty, so this needs a real loader **and a font binary the bundle does not ship** — decide the typeface before starting. (c) The 7-role scale does not cover Phase 3; see the new `THEME_MIGRATION.md` §4b and settle the approach here, not screen by screen._
