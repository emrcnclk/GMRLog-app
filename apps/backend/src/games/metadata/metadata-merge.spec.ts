...
describe('toMediaJob', () => {
  it('promotes a hero and a cover, and nothing else', () => {
    const job = (kind: 'cover' | 'hero' | 'artwork' | 'screenshot') =>
      toMediaJob('game-1', 'igdb', {
        kind,
        url: 'https://img/x.jpg',
        width: 1,
        height: 1,
    ...
    expect(job('hero').promote).toBe(true);
    expect(job('cover').promote).toBe(true);
    expect(job('screenshot').promote).toBe(false);
    expect(job('artwork').promote).toBe(false);
  });
});
